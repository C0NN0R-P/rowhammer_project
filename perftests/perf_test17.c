...
int main() {
  uint64_t umasks[NUM_UMASKS] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f);
  int fds[NUM_UMASKS];
  ...
  for (int i = 0; i < NUM_UMASKS; i++) {
    ...
    printf("\nUmasks=0x%llx => config=0x%llx", (long long)umasks[i], (long long)((umasks[i] << 8) | EVENT CODE));
